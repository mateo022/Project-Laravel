<x-app-layout>
  ...
  <div class="card-body">
    ...
    <form action="/categories/{{$category->slug}}"   method="post">
            @csrf
            @method('DELETE')
             <button type="submit" class="btn-danger" onclick="return confirm('¿Are you sure you want to delete the category'+' '+ '{{$category->slug}}?')" title="Delete Market" style="font-size: 30px"><i class="bi bi-trash"></i></button>
        </form>
  </div>    
  </div>
</div>
